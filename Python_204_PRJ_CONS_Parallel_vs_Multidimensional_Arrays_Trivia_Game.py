#Title: QUIZ - Parallel vs. 2D Array List objects
...
def QUIZ_PARALLEL_ARRAY():
    ...
    while(x < len(Questions)):
        ...
        UsersInput[x] = input("Answer? ").lower();
        ...
        if(GuessedIt == True or WrongPerQuestionCount >= MaxWrongGuesses):
           #Reset wrong counter for next question if exceeded max num guesses or player is correct
           print("\nResetting wrong question counter and boolean variable.");
           GuessedIt = False; #reset
           WrongPerQuestionCount = 0; #reset

        x += 1;
        NULL = input("Press \"ENTER\" to continue.");  
          
    print("\nParallel List/Array QUIZ complete.");
    FinalScore = Score * 20;
